(() => {
  ...
  function initialiseTeacherAuth() {
    ...
    teacherAuth.unsubscribe = window.firebase.auth().onAuthStateChanged(async (user) => {
      ...
      try {
        ...
        const adminSnap = await db.ref(`roles/admins/${user.uid}`).once("value");
        if (adminSnap.val() === true) {
          teacherAuth.role = "admin";
          teacherAuth.teacherSchools = [];
        } else {
          const teacherSnap = await db.ref(`roles/teachers/${user.uid}`).once("value");
  ...
  async function teacherLogin() {
    ...
    try {
      ...
      await withTimeout(window.firebase.auth().signInWithEmailAndPassword(email, password), 15000, getAuthTimeoutHelpMessage());

(() => {
  ...
  const MULTITHER_BUILD_VERSION = "v58 Verified Teacher/Admin Export";
  ...
  function applyLanguage() {
    ...
    setText("teacherExportHelp", `${t("teacherExportHelp")} Build: ${MULTITHER_BUILD_VERSION}.`);
  ...
  async function teacherLogin() {
    ...
    // v58: If Firebase already has this user signed in, do not wait for another sign-in request.
    // Reload the role directly. This prevents the status badge from staying at "Signing in..."
    // when browsers reuse an existing Auth session.
    const existingUser = window.firebase.auth().currentUser;
    if (existingUser && (!existingUser.email || existingUser.email.toLowerCase() === email.toLowerCase())) {
      ...
      setTeacherAuthStatus(t("teacherRoleLoading"), "loading");
      await loadTeacherRole(existingUser);
      return;
    }
